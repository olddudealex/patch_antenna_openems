...
- Loads materials from `line_array_openEMS_simulation/`:
...
1. **Expected STL Files** in `./line_array_openEMS_simulation/`:
...
2. **Mesh Line Extraction**:
   ...
   - Generates `meshlines_{x,y,z}.csv` in working directory
   - Axis-aligned edges extracted with configurable tolerances

3. **Grid Application**:
   - `apply_meshlines_from_csv()` adds coordinates to CSXCAD mesh
...
[STL Import] ──→ extract_edges_from_stl ──┤
    ↓            ↓                        │
stl_model.py   meshlines_*.csv           │
    ↓            ↓                        │
[Apply Mesh] ← apply_meshlines_from_csv ─┘
    ↓
[FDTD Solver] ──→ openEMS ──→ HDF5 dumps
    ↓
...
[Visualize] ────────────→ plot_s11(), plot_smith_skrf()
    ...
    │                    plot_directivity_*()
    ↓                         ↓
[Export] ────────────────→ PDF Report + .s1p/.s2p files
```
...
- Check STL file paths are valid and contain proper meshes
...
├── line_array_openEMS_simulation/      # STL models directory
│   ├── top_gen_model.stl
│   ├── bottom_gen_model.stl
│   └── substrate_gen_model.stl
├── meshlines_x.csv                     # Generated mesh coordinates
├── meshlines_y.csv
├── meshlines_z.csv
├── *.s1p, *.s2p                        # S-parameter exports
