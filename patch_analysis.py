...
# Half-wavelength line between adjacent patch elements
len180 = 12.9  # slightly shorter than half-wavelength

for sweep_idx in range(0, sweep_number):
    # General parameter setup
    Sim_Path = os.path.join(tempfile.gettempdir(), f"patch_analysis_{sweep_idx}")

    # FDTD setup
    FDTD = openEMS(NrTS=60000, EndCriteria=1e-6)
    ...
    start = [-feed_width/2, -patch_length/2, substrate_thickness]
    stop = [feed_width/2, -patch_length/2-len180, substrate_thickness]
    ...
    feed_pos = -patch_length / 2 - len180
    ...
    # increase density
    mesh.SmoothMeshLines('all', mesh_res, 1.01)
    ...
    filename = f"length={patch_length}mm width={patch_width}mm results.pdf"
    ...
    if draw_smith_chart:
        ax, meta = plot_smith_skrf(
            ...
            patch_width_mm=patch_width, patch_length_mm=patch_length, len180_mm=len180,
    ...
    if draw_Ez_absolute or draw_Ez_snap:
        ...
        E_fd = td_to_fd_dft(fd.E_td, fd.time, fd.dt, freq[freqInd])  # -> (Nx, Ny, Nz, 3)
    ...
    if draw_Ez_snap:
        ix, iy1, iz = coord_to_index(x=0, y=-patch_length/2000, z=0.0008, fd=fd)
        _, iy2, _ = coord_to_index(x=0, y=patch_length/2000, z=0.0008, fd=fd)
        align_weights = np.zeros(fd.y.shape[0])
        align_weights[iy1:iy2] = 1

        if sweep_idx == 0:
            reference_line = E_fd[ix, :, iz, 2]

        _, dphi = align_phase_global(E_fd[ix, :, iz, 2], reference_line, align_weights)
